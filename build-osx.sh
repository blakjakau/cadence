...
LOCAL_REPO_DIR="/home/jason/repo/dev.jakbox.cadence"
...
ssh "$REMOTE_USER@$REMOTE_HOST" << EOF
  ...
  if [ $RECOMPILE -eq 1 ]; then
    ...
    GOOS=darwin GOARCH=amd64 go build -ldflags="-s -w" -o cadence-macos-x64 .
    echo "Building for macOS arm64..."
    GOOS=darwin GOARCH=arm64 go build -ldflags="-s -w" -o cadence-macos-arm64 .
...
EOPLIST
  ...
  binaries_to_sign=("cadence-macos-x64" "cadence-macos-arm64")
...
scp "$REMOTE_USER@$REMOTE_HOST:$REMOTE_BUILD_DIR/cadence-macos-x64" "$LOCAL_DIST_DIR/"
scp "$REMOTE_USER@$REMOTE_HOST:$REMOTE_BUILD_DIR/cadence-macos-arm64" "$LOCAL_DIST_DIR/"
if [ $? -ne 0 ]; then
