...
export class FileList extends ContentFill {
	...
	async generateIndex(src) {
        if (this.disableIndexing) return null;
	...
	_render(base, tree, depth=0) {
		...
		tree?.forEach((item) => {
		    if (this.hideDotFiles && item.name.startsWith(".")) return;
	...
	set active(v) {
		const target = typeof v === 'string' ? v : (v instanceof FileSystemFileHandle ? buildPath(v) : null);
		if (!target) return;
